...
for graph in tqdm(graphs):
    ...
    for i in range(100):
        run = pd.read_csv(f'results/alpha1_00-beta0_50/convergence-{graph}-run{i}.tsv', sep='\t', usecols=['nMessages','meanHammingDist'])
        data.iloc[i] = run.iloc[0]

    data.to_csv(f'results/alpha1_00-beta0_50/merged/convergence-{graph}.tsv', sep='\t',index=False)
