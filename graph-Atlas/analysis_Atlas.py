...
if __name__ == "__main__":
    ...
    for i, index in enumerate(reversed(data['index'].unique())):
        ...
        ax.scatter(np.mean(data['clustering'].iloc[indices]),np.mean(data['nMessages'].iloc[indices]),color=color, alpha=0.5)
    ...
    for n in reversed(data['nodes'].unique()):
        ...
        # if n != 2:
        #     p = np.poly1d(np.polyfit(meanData['clustering'].iloc[indices],meanData['nMessages'].iloc[indices],3))
        #     t = np.linspace(min(meanData['clustering']), max(meanData['clustering']), 250)
        #     print(p)
        #     ax.plot(t,p(t),color)
        # """Uncomment for polynomial"""
